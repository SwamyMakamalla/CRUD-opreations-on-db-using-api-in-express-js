...
const convertDirectorObjectIntoResponseObject = (dbObject) => {
  return {
    directorId: dbObject.director_id,
...
//get a movie with movieId api get
...
app.get("/directors/", async (request, response) => {
  ...
  response.send(
    directorsArray.map((eachDirector) =>
      convertDirectorObjectIntoResponseObject(eachDirector)
    )
  );
});
...
app.get("/directors/:directorId/movies/", async (request, response) => {
  ...
  const movies = await database.all(directorMoviesQuery);
  response.send(
